!    RMSDSPECBTR - Computes RMSd in a particular way for BTR files
...
integer*4 i,j,k,narg,xd,ret,arg,iprec,ipreci,ndxn,minframe,mmf,reffrm
integer*4 xda,kode,num,ll(256),ul(256)
character*10000 filename(5),line
...
call readarg('Reference Frame Number: ',narg,arg,line)

if (len_trim(line).le.0) then
  reffrm=1
else
  read(line,*) reffrm
endif
...
prname='RNSDSPECBTR'
prver='version 1.0'
prdesc='Computes RMSd in a particular way for BTR files'
author='Pablo M. De Biase (pablodebiase@gmail.com)'
startdate='06 Mar 2014'
lastdate='06 Mar 2014'
